...
```bash
npm run contract -- --novo    # cria a partir do modelo
npm run contract              # valida
```
...
4. **Gates rápidos rodam sozinhos.** O hook `Stop` em `.kiro/hooks/gates.json`
   dispara `npm run gates:fast` quando o agente acha que terminou. A definição
...
5. **Você revisa o diff, não a conversa.** `git diff --cached` antes de todo push
   ...
   shell a um agente, e é por isso que `npm run secrets` existe.
...
| Contexto do domínio | `.kiro/steering/product.md` | tudo |
