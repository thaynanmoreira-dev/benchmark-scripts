...
```bash
bash install.sh /caminho/do/servico                   # serviço novo
MODE=ratchet bash install.sh /caminho/do/servico      # serviço que já existe
```
...
- **Clean Architecture com quatro camadas por módulo** (`domain`, `application`,
  ...
  outra, ajuste `.kiro/steering/structure.md` e `.dependency-cruiser.cjs`
...
`.kiro/steering/product.md`, que vem como template para você preencher.
...
`npm run gates:fast` quando o agente acha que terminou, então "esqueci de
...
ligado entra em todo prompt e vira custo de crédito recorrente. Se `product.md` e
...
| Regra | Ferramenta | Config |
|---|---|---|
| Contrato da tarefa completo | script próprio | `tools/contract.mjs` |
...
| Nenhuma supressão nova | script próprio | `tools/no-bypass.mjs` |
| Nenhum gate afrouxado ou apagado | script próprio | `tools/gates-intact.mjs` |
...
```bash
node tools/no-bypass.mjs --strict    # serviço novo: zero tolerância
node tools/no-bypass.mjs --gravar    # serviço existente: fixa a linha de base
node tools/no-bypass.mjs             # CI: reprova aumento
```
...
Limite honesto: o próprio `gates-intact.mjs` é um arquivo, e quem editar o
